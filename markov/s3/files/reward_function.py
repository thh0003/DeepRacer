...
class RewardFunction():
    ...
    def _download(self):
		...
		# Additional File for reward function
		rewardfiles = [
			"tboneDeepRacerHistory.py",
			"tboneDeepRacerRaceStep.py",
			"tboneDeepRacerUtils.py"
		]
        ...
        try:
			...
			print("s3 key: ")
			print(self._s3_key)
			for rwFile in rewardfiles:
				fileNameIndex = self._s3_key.rfind("/")
				curS3Key = self._s3_key[:fileNameIndex]+rwFile
				print("current file: ")
				print(curS3Key)
				self._s3_client.download_file(bucket=self._bucket,
											s3_key=curS3Key,
